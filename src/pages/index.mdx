...
The [tttm] is a game of [tic-tac-toe][ttt] played between APIs that players implement themselves.

## Rules

1. Players can use whatever language and whatever technology.
2. The player must submit the URL to a server via the PR request to the [tttm repository](https://github.com/ogrodje/tttm/pulls).
3. All players and their servers are listed in [players.yml](https://github.com/ogrodje/tttm/blob/master/players.yml).
4. The player can submit as many servers as he or she wants.
...
- It can be used on players servers to stick together individual games.
- `size` - A number - size - of [tic-tac-toe][ttt] grid.
    - By default, the size is set to `3`, representing the grid of size `3x3`.
    - To win a game one has to have 3 symbols in either row/column/diagonal.
    - ⚠️ The `tttm` engine currently only supports a size of 3.
    - ⚠️ We will expand this to `5x5` and `7x7`, where one must put four symbols to win.
